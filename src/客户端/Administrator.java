...
import java.sql.SQLException;
public class Administrator extends User implements ActionListener,DocumentListener,ListSelectionListener ,Serializable{
	Dimension dim;
...
public void valueChanged(ListSelectionEvent arg0) {
	...
	// TODO 自动生成的方法存根
		if(arg0.getSource().equals(userList.getSelectionModel()))
		changeUserInfoUi();
...
void UpdateuserList() {
	...
	GetnowUserList(false);
	 DefaultTableModel lmodel=(DefaultTableModel) userList.getModel();
	 lmodel.getDataVector().clear();
	 for(int i=0;i<nowuserList.size();i++) {
		   lmodel.addRow(((User) nowuserList.get(i)).UserObject());
	    }
	 userList.repaint();
}
void UpdateFileList() {
...
private void changeUserInfoUi() {
	// TODO 自动生成的方法存根
	 ...
	 DefaultTableModel lmodel=(DefaultTableModel) userList.getModel();
	 try {
	 user=(User) nowuserList.get(userList.getSelectedRow());
}catch(Exception e) {
	 }
	 if(user!=null) {
...
private void GetnowUserList(boolean choose) {//1表示所有，0表示实时
	// TODO 自动生成的方法存根
	if(choose)nowfileList=all;
	else	{
		nowuserList=new ArrayList<User>();
		for(int i=0;i<all.size();i++)
			if(all.get(i).getName().contains(textuserName.getText()))
				nowuserList.add(all.get(i));
		}}
...
private void DisplayUserList() {
   ...
   for(int i=0;i<nowuserList.size();i++) {
	   model.addRow(((User) nowuserList.get(i)).UserObject());
    }
